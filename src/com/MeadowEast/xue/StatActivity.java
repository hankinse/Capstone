...
public class StatActivity extends Activity {
	...
	public static final int TIME_HORIZON_DAYS = 7; // The number of days we want
	...
	private void setStatus(String name, TextView date, TextView level,
			TextView week) {
		...
		if (weekLog.size() < 1) {
			...
			levels = weekLog.get(weekLog.size() - 1).split(" "); // The last
																	// line
																	// contains
																	// the most
																	// recent.
		...
		if (hasLog) {
			...
			level.setText("Level 0: " + levels[0] + "\n" + "Level 1: "
					+ levels[1] + "\n" + "Level 2: " + levels[2] + "\n"
					+ "Level 3: " + levels[3] + "\n" + "Level 4: " + levels[4]);
			// We want to check to see if it's still too early to extract
			// meaningful weekly results
			if (numCompletedDecks < NUM_DECKS_UNTIL_REPORT) {
				String progress = "Here you will see a report of your learning progress after you have completed at least "
						+ NUM_DECKS_UNTIL_REPORT
						+ " decks.  We consider a card learned after it reaches level 2.  "
	...
	private ArrayList<String> getRelevantEntries(String name) {
		...
		weekAgoCalendar.add(Calendar.DAY_OF_MONTH, -TIME_HORIZON_DAYS); // Subtract
																		// time_horizon
																		// days
																		// from
																		// right
		...
		try {
			File logFileHandle = new File(MainActivity.filesDir, name
					+ ".log.txt");
			...
			while ((currentLine = reader.readLine()) != null) {
				...
				if (date != null) {
					...
					if (weekAgo.before(date)) {
						...
						// This is highly tailored to the exact file we're
						// working with.
						levelCount = new String(tokens.get(0) + " "
								+ tokens.get(1) + " " + tokens.get(3) + " "
								+ tokens.get(6) + " " + tokens.get(8));
						out.add(levelCount);
			...
			Log.d(TAG, "Error reading log file.");
			hasLog = false;
		}
	...
	private String extractWeeklyResults(String start, String end, int numDecks) {
		...
		log = new String("In the past week, you have completed " + numDecks
				+ " decks and " + numLearned + "for an average of " + average
				+ " per day.");
